...
my $magneticKeyboard = `/bin/awk \'c&&!--c;/STM32/{c=4};\' /proc/bus/input/devices | /bin/awk \'{print \$4}\'`;

$rfidKeyboard = untaint('kbd', $rfidKeyboard);
$magneticKeyboard = untaint('kbd', $magneticKeyboard);
    
if($rfidKeyboard and $magneticKeyboard) {
    print("RFID keyboard: $rfidKeyboard\n");
    print("Magnetic keyboard: $magneticKeyboard\n");

    my @posArgs = split(/\s+/, "/opt/re-vend/pos.py --rfidKeyboard /dev/input/$rfidKeyboard --magneticKeyboard /dev/input/$magneticKeyboard");
    system("/usr/bin/python @posArgs 1>/var/log/revend/pos 2>&1");
}
else {
